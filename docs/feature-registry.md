...
| Grid snap (Ctrl modifier) | ✅ Complete | `Ctrl` held during any drag type; snaps to `editors/3d/grid_step` from EditorSettings via `_get_snap_step()`; applied in all drag types.  Toolbar: plain **Snap** button → popup with enum dropdowns (Mode / Translate / Rotation / Scale) plus a read-only summary label; **Local/World** gizmo Space on its own OptionButton.  Snap Mode: **Hybrid** (ProBuilder default — object moves snap final world positions to grid crossings, element edits + scale/extrude quantize deltas), **World** (translate deltas snap to grid increments — rigid, aligned edges/corners stay on grid; scale quantizes the resulting world size), **Delta** (legacy local incremental) |
