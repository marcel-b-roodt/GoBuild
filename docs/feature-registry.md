...
| Vertex color paint | 🔧 In Progress | `VertexColorOperation`: fill_faces, fill_all, set_vertices with blend modes (Mix/Add/Subtract/Multiply) and channel masking (R/G/B/A); `GoBuildVertexPainter` separate dock panel with colour picker, channel toggles, blend mode, brush radius/strength, Fill Selected/Fill All buttons; eyedropper button (placeholder); data model and ARRAY_COLOR bake pre-existing; `GoBuildVertexColorDrawer` removed (replaced by painter panel)
