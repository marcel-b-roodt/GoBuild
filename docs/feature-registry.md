...
| Vertex color paint | 🔧 In Progress | `VertexColorOperation`: fill_faces, fill_all, set_vertices with blend modes (Mix/Add/Subtract/Multiply) and channel masking (R/G/B/A); `GoBuildVertexPainter` separate dock panel with colour picker, channel toggles, blend mode, brush radius/strength, Fill Selected/Fill All buttons; eyedropper button (placeholder); vertex_colors preserved through all mesh operations (extrude, bevel, inset, loop-cut, subdivide, rip, edge-extrude) with lerp/inherit for new vertices; `_sync_vertex_colors()` safety net in `rebuild_edges()`
