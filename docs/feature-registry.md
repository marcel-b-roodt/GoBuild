...
| Localized snap grid (Ctrl overlay) | ✅ Complete | Three small orthogonal panels (±2 snap cells) at the mode-correct anchor during translate drags with Ctrl: bright axis centre lines + dim outline + lattice-point crosses; Godot axis colours; Delta mode orients to node-local axes |
| Grid snap (Ctrl modifier) | ✅ Complete | `Ctrl` held during any drag type; snaps to `editors/3d/grid_step` from EditorSettings via `_get_snap_step()`; applied in all drag types.  GoBuild strip in the 3D editor menu bar: **Snap** button → popup with enum dropdowns (Mode / Translate / Rotation / Scale), read-only summary label, **Local/World** gizmo-space OptionButton, and a cog (⚙) menu holding Print Selection, Debug Logging, X-Ray, Face/Vertex Normals, and Reset Panel Layout. The global Debug/X-Ray/Normals toggles moved out of the General drawer into the cog menu.  Snap Mode: **Hybrid** (ProBuilder default — object moves snap final world positions to grid crossings, element edits + scale/extrude quantize deltas), **World** (translate deltas snap to grid increments — rigid, aligned edges/corners stay on grid; scale quantizes the resulting world size), **Delta** (legacy local incremental) |
