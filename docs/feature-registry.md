...
| Grid snap (Ctrl modifier) | ✅ Complete | `Ctrl` held during any drag type; snaps to `editors/3d/grid_step` from EditorSettings via `_get_snap_step()`; applied in all drag types.  Toolbar **Snap Mode**: **Hybrid** (ProBuilder default — translate snaps final world positions to grid crossings, scale quantizes the ratio / extrude depths quantize) or **World** (translate deltas snap to grid increments — rigid, aligned edges/corners stay on grid; scale quantizes the resulting world size) |
