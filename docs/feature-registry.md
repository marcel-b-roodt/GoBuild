...
| Grid snap (Ctrl modifier) | ✅ Complete | `Ctrl` held during any drag type; snaps to `editors/3d/grid_step` from EditorSettings via `_get_snap_step()`; applied in all drag types.  GoBuild strip in the 3D editor menu bar: **edit-mode toggles**, **Help** icon, **Snap** button (chevron) → popup with enum dropdowns (Mode / Translate / Rotation / Scale), read-only summary label, **Local/World** gizmo-space OptionButton, cog (⚙) menu (Print Selection, Debug Logging, X-Ray, Face/Vertex Normals, Reset Panel Layout), **edit-mode toggles** (Object/Vertex/Edge/Face, synced with shortcuts), and **Help** (cheatsheet popup). The global Debug/X-Ray/Normals toggles moved out of the General drawer into the cog menu; the dock panel keeps its drawers and context label only.  Snap Mode: **Hybrid** (ProBuilder default — object moves snap final world positions to grid crossings, element edits + scale/extrude quantize deltas), **World** (translate deltas snap to grid increments — rigid, aligned edges/corners stay on grid; scale quantizes the resulting world size), **Delta** (legacy local incremental) |
