...
| Vertex color paint | 🔧 In Progress | `VertexColorOperation`: fill_faces, fill_all, set_vertices with blend modes (Mix/Add/Subtract/Multiply) and channel masking (R/G/B/A); `GoBuildVertexPainter` separate dock panel with colour picker, channel toggles, blend mode, brush radius/strength, Fill Selected/Fill All buttons, Paint mode toggle (LMB paints when active, normal editing when off); `GoBuildVertexPaintBrush` stroke controller: click-drag to paint vertices within brush radius, F=resize, S=strength, A=cycle blend, Ctrl+click eyedropper, undo/redo per stroke; brush cursor overlay with radius circle and info label; `paint_vertices_in_radius` static helper for testable brush logic; `begin_preview`/`bake_preview`/`end_preview` for performant in-stroke updates; auto-transparency when vertex alpha < 1.0; channel visualizer (R/G/B/A greyscale); template blend shader (`vertex_blend_mask.gdshader`); vertex_colors preserved through all mesh operations; `_sync_vertex_colors()` safety net in `rebuild_edges()` |
