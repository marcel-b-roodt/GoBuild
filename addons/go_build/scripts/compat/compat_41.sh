#!/usr/bin/env bash
# compat_41.sh — Make GoBuild compatible with Godot 4.1.
#
# NOTE: GoBuild uses EditorInterface static methods that were instance-only
# in 4.1. A text-only compat script cannot fix this. Godot 4.2 is the
# practical minimum supported version. This script is kept for reference
# but is NOT recommended for actual 4.1 use.
#
# Applies transforms that are mechanically possible:
#   1. Strip typed for-loop variables              (4.2+ syntax)
#   2. Replace DOCK_SLOT_BOTTOM with integer 7     (4.3+ constant)
#   3. Replace DisableMode enum                    (4.3+ — use int constants instead)
#   4. Replace Vector3.min()/max() with minf/maxf  (4.3+ methods)
#   5. Replace "as int" enum casts with int()      (4.3+ cast syntax)
#
# Does NOT fix EditorInterface API changes (static vs instance methods).
# Godot 4.2 is the recommended minimum version.
#
...
echo "GoBuild — Godot 4.1 compat (in-place) — PARTIAL"
echo "Target: ${ADDON_ROOT}"
echo ""
echo "WARNING: GoBuild uses EditorInterface static methods that were"
echo "         instance-only in Godot 4.1. This script CANNOT fix those."
echo "         Godot 4.2 is the recommended minimum version."
echo "         The script will still apply the transforms below."
echo ""
...
echo ""
echo "Done. The addon at ${ADDON_ROOT} has partial 4.1 compatibility."
echo "EditorInterface API changes remain — Godot 4.2 is the recommended minimum."
