#!/usr/bin/env bash
# compat_40.sh — Make GoBuild compatible with Godot 4.0.
#
# Applies all transforms needed to run GoBuild on Godot 4.0:
#   1. Strip typed for-loop variables              (4.2+ syntax)
#   2. Strip @export_group/subgroup               (4.1+ feature)
#   3. Replace DOCK_SLOT_BOTTOM with integer 7     (4.3+ constant)
#   4. Replace DisableMode enum                    (4.3+ enum)
#   5. Replace Vector3.min()/max() with minf/maxf  (4.3+ methods)
#   6. Replace "as int" enum casts with int()      (4.3+ cast syntax)
#
...
# Users should run this on an installed copy, not the repo itself.
...
source "${SCRIPT_DIR}/functions/strip_export_groups.sh"
...
echo "GoBuild — Godot 4.0 compat (in-place)"
...
echo "→ Stripping @export_group and @export_subgroup (4.1+ feature)..."
compat::strip_export_groups "${ADDON_ROOT}"
...
echo ""
echo "Done. The addon at ${ADDON_ROOT} is now 4.0-compatible."
echo "NOTE: EditorInterface API changes remain — 4.2 is the practical minimum."
