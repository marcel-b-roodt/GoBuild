...
##
## When [param inner_centroids] is provided, it is populated with a mapping
## from each new inner vertex index to its face centroid (local space).  Used
...
## (0 = no inset, 1 = fully collapsed).
## [param inner_centroids] is optionally populated:
## inner_vert_idx → local centroid.
